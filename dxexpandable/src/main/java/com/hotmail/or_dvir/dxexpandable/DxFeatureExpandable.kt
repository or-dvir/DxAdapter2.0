...
class DxFeatureExpandable<ITEM : IDxBaseItem>(
    ...
    override val onItemClick: OnItemClickListener<IDxBaseItem> =
        { view, adapterPosition, item ->
            ...
            if (item is IDxItemExpandable && isInSelectionMode()) {
    ...
    private fun selectOrDeselect(shouldSelect: Boolean, items: List<ITEM>) {
        ...
        items.forEach {
            ...
            if (it is IDxItemExpandable && shouldSelect != it.isSelected) {
    ...
    fun getAllSelectedItems() =
        adapter.getDxAdapterItems().filter { it is IDxItemExpandable && it.isSelected }
